...
SINGLE_BATTLE_TEST("Leaf Guard prevents non-volatile status conditions in sun")
{
    ...
    GIVEN {
        ...
        PLAYER(SPECIES_LEAFEON) { Ability(ABILITY_LEAF_GUARD || ABILITY_MEGA_BURST_VENUSAUR); }
...
SINGLE_BATTLE_TEST("Leaf Guard prevents status conditions from Flame Orb and Toxic Orb")
{
    ...
    GIVEN {
        ...
        PLAYER(SPECIES_LEAFEON) { Ability(ABILITY_LEAF_GUARD || ABILITY_MEGA_BURST_VENUSAUR); Item(item); }
        OPPONENT(SPECIES_WOBBUFFET);
    } WHEN {
...
SINGLE_BATTLE_TEST("Leaf Guard prevents Rest during sun")
{
    GIVEN {
        ...
        PLAYER(SPECIES_LEAFEON) { Ability(ABILITY_LEAF_GUARD || ABILITY_MEGA_BURST_VENUSAUR); HP(100); MaxHP(200); }
        OPPONENT(SPECIES_WOBBUFFET);
    } WHEN {
